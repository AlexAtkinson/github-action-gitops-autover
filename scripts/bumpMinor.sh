...
merge_branch="feature/XX-BUMP_COUNT_MinorBump"

case "$origin_host" in
  github)
    merge_string="Merge pull request #NA from org/feature/XX-BUMP_COUNT_MinorBump"
  ;;
  gitlab)
    merge_string="Merge branch 'feature/XX-BUMP_COUNT_MinorBump' into '$main'"
  ;;
  bitbucket)
    merge_string="Merged in bugfix/XX-BUMP_COUNT_MinorBump (pull request #NA)"
  ;;
  *)
...
for i in $(eval echo "{1..$1}") ; do
  ...
  git commit --allow-empty -m "Minor Version Bump"
  git checkout $main
